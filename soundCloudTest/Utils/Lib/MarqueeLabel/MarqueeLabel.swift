...
class MarqueeLabel: UILabel, CAAnimationDelegate {
    ...
    var type: MarqueeType = .continuous {
        didSet {
            if type == oldValue {
                return
            }
    ...
    @IBInspectable var labelize: Bool = false {
        didSet {
    ...
    @IBInspectable var holdScrolling: Bool = false {
        didSet {
    ...
    @IBInspectable var tapToScroll: Bool = false {
        didSet {
    ...
    var speed: SpeedLimit = .duration(7.0) {
        didSet {
    ...
    @IBInspectable var scrollDuration: CGFloat {
        get {
            switch speed {
    ...
    @available(*, deprecated, message : "Use speed property instead")
    @IBInspectable var scrollRate: CGFloat {
        get {
            switch speed {
    ...
    @IBInspectable var leadingBuffer: CGFloat = 0.0 {
        didSet {
    ...
    @IBInspectable var fadeLength: CGFloat = 0.0 {
        didSet {
    ...
    @IBInspectable var animationDelay: CGFloat = 1.0
    ...
    @objc func restartForViewController(_ notification: Notification) {
    ...
    @objc func labelizeForController(_ notification: Notification) {
        if let controller = (notification as NSNotification).userInfo?["controller"] as? UIViewController {
            if controller === self.firstAvailableViewController() {
                self.labelize = true
            }
    ...
    @objc func animateForController(_ notification: Notification) {
        if let controller = (notification as NSNotification).userInfo?["controller"] as? UIViewController {
            if controller === self.firstAvailableViewController() {
                self.labelize = false
            }
    ...
    func labelWillBeginScroll() {
        // Default implementation does nothing - override to customize
        return
    }
    
    func labelReturnedToHome(_ finished: Bool) {
        // Default implementation does nothing - override to customize
        return
    }
...
struct ScrollStep: MarqueeStep {
    ...
    let timingFunction: UIView.AnimationCurve
...
struct EdgeFade: OptionSet {
    let rawValue: Int
    static let leading = EdgeFade(rawValue: 1 << 0)
    static let trailing = EdgeFade(rawValue: 1 << 1)
